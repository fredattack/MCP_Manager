...
import { CanvasContent, CanvasPanelProps, ExportFormat } from '@/types/ai/claude.types';
...
export const CanvasPanel: FC<CanvasPanelProps> = ({ className, selectedMessage, content }) => {
  ...
  ${convertToHtml(extractedContent)}
    ...
    const renderContent = () => {
        ...
        switch (canvasContent.type) {
            case 'code':
                return (
                    <CodeBlock code={canvasContent.content} language={canvasContent.metadata?.language} showLineNumbers={true} className="h-full" />
                );

            case 'markdown':
                return <MarkdownRenderer content={canvasContent.content} className="prose prose-sm dark:prose-invert max-w-none" />;

            case 'table':
                return <TableRenderer content={canvasContent.content} className="w-full" />;

            case 'mixed':
                return (
                    <div className="space-y-4">
                        <MarkdownRenderer content={canvasContent.content} className="prose prose-sm dark:prose-invert max-w-none" />
                    </div>
                );
            ...
            default:
                return (
                    <div className="p-4">
                        <pre className="text-sm whitespace-pre-wrap text-gray-700 dark:text-gray-300">{canvasContent.content}</pre>
                    </div>
                );
        }
    };
    ...
    return (
        <div className={cn('flex h-full flex-col bg-gray-50 dark:bg-gray-800', isFullscreen && 'fixed inset-0 z-50', className)}>
            {/* Header */}
            <div className="border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-900">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {/* Zoom controls */}
                        <div className="mr-2 flex items-center gap-1 border-r border-gray-200 pr-2 dark:border-gray-700">
                            <button
                            ...
                            <span className="min-w-[3rem] text-center text-xs text-gray-600 dark:text-gray-400">{zoom}%</span>
                            <button
                        ...
                        <div className="group relative">
                            ...
                            <div className="invisible absolute right-0 z-10 mt-1 w-48 rounded-md bg-white opacity-0 shadow-lg transition-all duration-200 group-hover:visible group-hover:opacity-100 dark:bg-gray-800">
                                <div className="py-1">
            ...
            {/* Content */}
            <div ref={contentRef} className="flex-1 overflow-auto p-6" style={{ fontSize: `${zoom}%` }}>
                {renderContent()}
            </div>
        </div>
    );
...
const EmptyCanvasPlaceholder: FC = () => (
    <div className="flex h-full items-center justify-center">
        <div className="max-w-md text-center">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Select an assistant message to view formatted content</p>
            <p className="mt-4 text-xs text-gray-500 dark:text-gray-500">The canvas automatically detects and formats:</p>
